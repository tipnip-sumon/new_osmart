...
<div class="row mb-4 my-4">
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.general-settings.social-media.update') }}" method="POST">
                    @csrf
                    ...
                    <ul class="nav nav-tabs mb-4" id="socialTabs" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="business-hours-tab" data-bs-toggle="tab" 
                    ...
                    <div class="tab-content" id="socialTabsContent">
                        ...
                        <!-- Business Hours Tab -->
                        <div class="tab-pane fade" id="business-hours" role="tabpanel">
                            @php
                                ...
                                $businessHours = array_merge($defaultHours, $businessHours);
                                $dayNames = [
                                    'monday' => 'Monday',
                                    'tuesday' => 'Tuesday',
                                    'wednesday' => 'Wednesday',
                            ...
                            <div class="row">
                                        ...
                                        <div class="card-body">
                                            @foreach($dayNames as $key => $dayName)
                                                <div class="row mb-3 align-items-center">
                                                    <div class="col-md-2">
                                                        <label class="form-label fw-semibold">{{ $dayName }}</label>
                                                    </div>
                                                    <div class="col-md-3">
                                                        <input type="time" 
                                                               class="form-control" 
                                                    ...
                                                    <div class="col-md-3">
                                                        <div class="form-check">
                                                            <input class="form-check-input" 
                                                                   type="checkbox" 
                                                                   name="business_hours[{{ $key }}][closed]"
                                                                   value="1"
                                                                   id="closed_{{ $key }}"
                                                                   {{ (!empty($businessHours[$key]['closed']) && $businessHours[$key]['closed']) ? 'checked' : '' }}
                                ...
                                <div class="col-lg-4">
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <div id="business-hours-preview">
                                                @foreach($dayNames as $key => $dayName)
                                                    <div class="d-flex justify-content-between mb-2" id="preview_{{ $key }}">
                                                        <span class="fw-semibold">{{ $dayName }}:</span>
                                                        <span class="text-muted" id="hours_{{ $key }}">
                                                            @if(!empty($businessHours[$key]['closed']) && $businessHours[$key]['closed'])
                                                                Closed
                                                            @else
                                                                {{ date('g:i A', strtotime($businessHours[$key]['open'] ?? '09:00')) }} - {{ date('g:i A', strtotime($businessHours[$key]['close'] ?? '17:00')) }}
                                                            @endif
                                                        </span>
                                                    </div>
                                                @endforeach
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateBusinessHoursPreview() {
        ...
        days.forEach(function(day) {
            ...
            const openInput = document.getElementById('open_' + day);
            const closeInput = document.getElementById('close_' + day);
            const previewElement = document.getElementById('hours_' + day);
            
            if (closedCheckbox && closedCheckbox.checked) {
                previewElement.textContent = 'Closed';
                previewElement.className = 'text-danger';
            } else {
                const openTime = formatTime(openInput.value);
                const closeTime = formatTime(closeInput.value);
                previewElement.textContent = openTime + ' - ' + closeTime;
    ...
    // Add event listeners for business hours inputs
    const days = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
    days.forEach(function(day) {
        const openInput = document.getElementById('open_' + day);
        const closeInput = document.getElementById('close_' + day);
        ...
        if (openInput) openInput.addEventListener('change', updateBusinessHoursPreview);
        if (closeInput) closeInput.addEventListener('change', updateBusinessHoursPreview);
        if (closedCheckbox) {
            // Initialize state
            toggleDayHours(day);
        }
