                        ...
                        <div class="col-md-9">
                            @php
                                ...
                                if ($popup->image_data) {
                                    ...
                                    if ($imageData && isset($imageData['sizes']['medium']['url'])) {
                            ...
                            @if($imageUrl)
                                <div class="mb-2">
                                    <img src="{{ $imageUrl }}" alt="Popup Image" class="img-thumbnail" style="max-width: 200px; max-height: 150px;">
                                </div>
                                <small class="text-muted">{{ $popup->image ?? ($imageData['original_name'] ?? 'Image') }}</small>
                            @else
                                <span class="text-muted">Image data available but URL not found</span>
                            @endif
                        </div>
                    </div>
                    @endif
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9">{{ ucfirst($popup->modal_size ?? 'Medium') }}</div>
                    </div>
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="row mb-2">
                        ...
                        <div class="col-6">{{ $popup->priority ?? 1 }}/10</div>
                    </div>
                    
                    <div class="row mb-2">
                        <div class="col-6"><strong>Frequency:</strong></div>
                        <div class="col-6">{{ ucfirst(str_replace('_', ' ', $popup->frequency ?? 'always')) }}</div>
                    </div>
                    
                    <div class="row mb-2">
                        <div class="col-6"><strong>Trigger:</strong></div>
                        <div class="col-6">{{ ucfirst(str_replace('_', ' ', $popup->trigger_type ?? 'immediate')) }}</div>
                    </div>
                    
                    @if($popup->trigger_value)
                    <div class="row mb-2">
                        <div class="col-6"><strong>Trigger Value:</strong></div>
                        <div class="col-6">{{ $popup->trigger_value }}</div>
                    </div>
                    @endif
                    ...
                    <div class="row mb-2">
                        <div class="col-6"><strong>Target Devices:</strong></div>
                        <div class="col-6">
                            @php
                                $devices = is_array($popup->target_devices) ? $popup->target_devices : json_decode($popup->target_devices, true);
                            @endphp
                            {{ implode(', ', $devices ?? ['all']) }}
                        </div>
                    </div>
                    ...
                    <div class="row mb-2">
                        <div class="col-6"><strong>Target Pages:</strong></div>
                        <div class="col-6">
                            @php
                                $pages = is_array($popup->target_pages) ? $popup->target_pages : json_decode($popup->target_pages, true);
                            @endphp
                            {{ implode(', ', $pages ?? []) }}
                        </div>
                    </div>
                    ...
                    <div class="small">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" disabled {{ ($popup->close_button ?? true) ? 'checked' : '' }}>
                            <label class="form-check-label">Closable</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" disabled {{ ($popup->overlay ?? true) ? 'checked' : '' }}>
                            <label class="form-check-label">Has Overlay</label>
                        </div>
                        <div class="form-check form-check-inline">
                ...
                <div class="card-body">
                    <div class="row text-center mb-3">
                        <div class="col-6">
                            <div class="p-3 border rounded">
                                <h4 class="text-primary mb-1">{{ $popup->displays ?? 0 }}</h4>
                                <small class="text-muted">Total Views</small>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="p-3 border rounded">
                                <h4 class="text-success mb-1">{{ $popup->clicks ?? 0 }}</h4>
                    ...
                    <div class="text-center">
                        <small class="text-muted">
                            Click Rate: {{ number_format((($popup->clicks ?? 0) / ($popup->displays ?? 1)) * 100, 1) }}%
                        </small>
                    </div>
                    @endif
                </div>
