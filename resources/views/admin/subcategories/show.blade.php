                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body text-center">
                        @if($subcategory['image'] ?? null)
                            ...
                            @endphp
                            <img src="{{ $imagePath }}" alt="{{ $subcategory['name'] ?? 'Subcategory' }}" 
                                 class="img-fluid rounded" style="max-height: 300px;"
                                 onerror="this.src='https://via.placeholder.com/300x200/e8f5e8/28a745?text=No+Image';">
                        @else
                            <div class="py-5">
...
<script>
    ...
    function duplicateSubcategory() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                    title: 'Duplicating...',
                    text: 'Please wait while we duplicate the subcategory.',
                    icon: 'info',
                ...
                // Make AJAX request to duplicate the subcategory
                fetch(`/admin/subcategories/${subcategoryId}/duplicate`, {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            title: 'Duplicated!',
                            text: `Subcategory has been duplicated successfully as "${data.duplicate_name}".`,
                            icon: 'success',
                            showCancelButton: true,
                            confirmButtonText: 'View Duplicate',
                            cancelButtonText: 'Stay Here'
                        }).then((result) => {
                            if (result.isConfirmed) {
                                // Redirect to the duplicate subcategory
                                window.location.href = `/admin/subcategories/${data.duplicate_id}`;
                            }
                            // If they choose to stay here, do nothing (no page reload)
                        });
                    } else {
                        Swal.fire('Error!', data.error || 'Failed to duplicate subcategory.', 'error');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    Swal.fire('Error!', 'An error occurred while duplicating subcategory.', 'error');
                });
            }
        });
    }
    ...
    function confirmStatusToggle() {
        ...
        .then(data => {
            if (data.success) {
                // Update the status badge in the UI without reloading
